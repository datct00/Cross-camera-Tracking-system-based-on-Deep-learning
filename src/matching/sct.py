...
TRACKER_ROOT_DIR = f"src/SCMT/tmp/validation"
...
def matching_camera(scene_camera, n_clusters=8, isVisualize=False):
    ...
    for line in f:
        ...
        #MOTChallenge format
        result.append(
            f"{frame_id},{track_matching[track_id]},{x},{y},{w},{h},-1,-1,-1,-1\n"
        )
    print(
        "Saving output single camera matching at",
        # f"outputs/matching/{scene_camera}.txt",
        f"outputs/matching/validation/{scene_camera}.txt",
    )
    # with open(f"outputs/matching/{scene_camera}.txt", "w") as f:
    with open(f"outputs/matching/validation/{scene_camera}.txt", "w") as f:
        f.writelines(result)
